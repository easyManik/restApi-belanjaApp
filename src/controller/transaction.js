...
const transactionModels = require('../model/transaction');
...
exports.getTransaction = async (req, res, next) => {
  try {
    ...
    const result = await transactionModels.selectTransaction({
      limit,
      offset,
      userId,
      sortby,
    ...
    const {
      rows: [count],
    } = await transactionModels.countTransaction(userId);
...
exports.updateTransaction = async (req, res, next) => {
  try {
    ...
    const { checkout_id, address, status, product_id, total, quantity } =
      req.body;
    const data = {
      checkout_id,
      userId,
